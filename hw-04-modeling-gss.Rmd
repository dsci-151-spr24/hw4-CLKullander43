...
```{r eval=TRUE}
gss16 <- gss16 %>%
  mutate(advfront=case_when(advfront == "Strongly agree" ~ "Agree", advfront == "Agree" ~ "Agree", advfront == "Disagree" ~ "Not agree", advfront == "Strongly disagree" ~ "Not agree", advfront == "Dont know" ~ "Not agree"))
```
...
```{r eval=TRUE}
gss16 <- gss16 %>%
  mutate(polviews=case_when(polviews=="Conservative" ~ "Conservative", polviews=="Slghtly conservative" ~ "Conservative", polviews=="Extrmly conservative" ~ "Conservative", polviews=="Liberal" ~ "Liberal", polviews=="Slightly liberal" ~ "Liberal", polviews=="Extremely liberal" ~ "Liberal", polviews=="Moderate" ~ "Moderate"))
```

```{r include=TRUE}
gss16 %>%
  arrange(polviews, "Conservative", "Moderate", "Liberal") %>%
  count(polviews)
```
...
```{r eval=TRUE}
gss16_rec_1 <- recipe(advfront ~., data = gss16_train) %>%
...
**The accuracy is very consistent throughout the folds, with just one of them about 5% away from the rest. The roc_auc is a little less consistent as there are several values varying from .60 to .64. The mean squared error is also relatviely consistent, only varying by about .035. The average accuracy is .836, while the average area under the roc curve is .625.**
...
```{r eval=TRUE}
gss16_train_2 <- gss16_train %>%
   select(advfront, educ, polviews)

gss16_test_2 <- gss16_test %>%
   select(advfront, educ, polviews)
...
```{r eval=TRUE}
gss16_rec_2 <- recipe(advfront ~., data = gss16_train_2) %>%
  step_dummy(all_nominal(), -all_outcomes())
```
...
```{r eval=TRUE}
set.seed(1112)
gss16_folds_2 <- vfold_cv(gss16_train_2, v = 5)

gss16_fit_rs_2 <- gss16_wflow_2 %>%
  fit_resamples(gss16_folds_2)
```
...
**The average accuracy is 84%, while the average area under to ROC curve is 64%**
...
**Model 2 works slightly better than model one based on the area under the ROC curve. That said, there is barely any difference between the two.** 
...
gss16_pred_2 <- predict(gss16_fit_2, gss16_test_2, type="prob") %>%
  bind_cols(gss16_test_2 %>% select(advfront))
```

```{r eval=TRUE}
gss16_pred_1 <- gss16_pred_1 %>%
  mutate(advfront=case_when(advfront=="Agree"~ 1, advfront=="Not agree"~ 0)) %>%
  mutate(advfront=as.factor(advfront))
...
**My previous answer still holds up. The second model performs slightly better.**
...
```{r}
gss16_harass5 <- gss16 %>%
  mutate("harass5" = case_when(harass5 == "No" ~ "No", harass5 == "Yes" ~ "Yes")) %>%
  filter(harass5 != "NA")
...
**It can create a multitude of samples from the original data and calculate the percentage of yes and no answers for each of those, giving us a range of estimates that we can use.**
...
**We can be 95% sure that the proportion of working Americans who have experienced harassment at work is between 15.3% and 19.3%.**
...
**I would expect a 90% confidence interval to be narrower than the other interval, because it would remove more of the outside observations, making it more precise.**
